...
export const LeadDetail: React.FC<LeadDetailProps> = ({ 
  ...
  if (!lead) {
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="h-full flex flex-col items-center justify-center bg-white/50 backdrop-blur-sm p-12 text-center">
        <motion.div initial={{ scale: 0.8, rotate: -10 }} animate={{ scale: 1, rotate: 3 }} transition={{ type: "spring", stiffness: 200, damping: 15 }} className="w-24 h-24 bg-white rounded-[32px] shadow-[0_20px_50px_rgba(0,0,0,0.05)] flex items-center justify-center mb-8 border border-slate-50"><FolderOpen className="text-brand-500" size={40} /></motion.div>
  ...
  const getActivityColor = (type: string, title?: string) => {
    if (type === 'note' || title === 'Note Added') return 'bg-accent-500';
    if (type === 'call') return 'bg-brand-600';
    return 'bg-slate-600';
  };
    ...
    <div className="h-full overflow-y-auto bg-white p-10 custom-scrollbar relative">
      ...
      <motion.div layout className="flex items-start justify-between mb-12">
          ...
          <AnimatePresence mode="wait">
            {isEditingProfile ? (
              <motion.div key="edit" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }} className="space-y-4 pt-2">
                <input className="text-3xl font-black text-slate-900 border-b-2 border-brand-500 focus:outline-none w-full bg-transparent" value={editData.name} onChange={e => setEditData({...editData, name: e.target.value})} />
                <div className="flex items-center gap-2">
                  ...
                  <input className="text-brand-600 font-black border-b border-slate-200 focus:outline-none bg-transparent uppercase text-sm tracking-wider" value={editData.company} onChange={e => setEditData({...editData, company: e.target.value})} />
                </div>
                <div className="flex gap-3 pt-2">
                  <button onClick={handleSaveProfile} className="px-6 py-2 bg-brand-600 text-white text-xs font-black uppercase tracking-widest rounded-xl shadow-lg shadow-brand-600/20 active:scale-95 transition-all">Save</button>
              ...
              <motion.div key="view" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} className="pt-2">
                ...
                <p className="text-slate-500 font-bold mb-6 flex items-center gap-2">{lead.role} <span className="w-1.5 h-1.5 rounded-full bg-slate-200"></span> <span className="text-brand-600 uppercase tracking-widest text-xs font-black">{lead.company}</span></p>
                <div className="flex flex-wrap gap-6 text-sm">
                  <div onClick={onDial} className="flex items-center gap-3 px-4 py-2 bg-slate-50 rounded-2xl text-slate-600 hover:bg-brand-50 hover:text-brand-600 transition-all cursor-pointer group border border-transparent hover:border-brand-100">
                    <Phone size={16} className="text-slate-400 group-hover:text-brand-500" />
                    <span className="font-bold">{lead.phone}</span>
                  </div>
                  <div onClick={onMessage} className="flex items-center gap-3 px-4 py-2 bg-slate-50 rounded-2xl text-slate-600 hover:bg-accent-50 hover:text-accent-600 transition-all cursor-pointer group border border-transparent hover:border-accent-100">
                    <MessageSquare size={16} className="text-slate-400 group-hover:text-accent-500" />
        ...
        <div className="flex items-center gap-3 relative">
           <button onClick={() => setShowOptions(!showOptions)} className={`p-3 rounded-2xl transition-all ${showOptions ? 'bg-brand-600 text-white shadow-lg' : 'text-slate-400 hover:bg-slate-50'}`}>
           ...
           <AnimatePresence>{showOptions && (
             <>
               ...
               <motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }} className="absolute right-0 top-full mt-2 w-56 bg-white rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.15)] border border-slate-100 z-50 py-3 overflow-hidden">
                 ...
                 <button onClick={() => {onDelete?.(lead.id); setShowOptions(false)}} className="w-full text-left px-6 py-3 text-sm font-bold text-accent-600 hover:bg-accent-50 flex items-center gap-3 transition-colors mt-1 border-t border-slate-50 pt-4">
      ...
      <div className="grid grid-cols-3 gap-6 mb-12">
        <StatCard label="Deal Value" value={`$${(lead.dealValue || 0).toLocaleString()}`} icon={<DollarSign size={14} />} color="text-brand-600" isEditing={editingStat === 'dealValue'} onEdit={() => startEditingStat('dealValue')} onSave={handleSaveStat} onCancel={() => setEditingStat(null)} inputValue={tempStatValue} onInputChange={setTempStatValue} />
      ...
      <div className="mb-12">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-lg font-black text-slate-900 uppercase tracking-widest flex items-center gap-3">
            <div className="w-2 h-2 bg-brand-500 rounded-full"></div>Activity Feed
          </h3>
          ...
          <div className="relative pl-4 space-y-8 before:absolute before:left-4 before:top-4 before:bottom-4 before:w-px before:bg-slate-100">
            {filteredActivities.map((activity, idx) => (
              <motion.div key={activity.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.1 }} className="relative pl-10">
                ...
                <div className="bg-slate-50/50 rounded-3xl p-6 border border-slate-50 hover:bg-white hover:shadow-xl hover:shadow-slate-200/50 transition-all group border-transparent hover:border-brand-100">
      ...
      <motion.div layout className="bg-slate-900 rounded-[40px] p-10 text-white shadow-2xl shadow-slate-900/20">
        <div className="flex items-center justify-between mb-8"><h3 className="text-lg font-black uppercase tracking-widest flex items-center gap-3"><div className="w-2 h-2 bg-brand-400 rounded-full animate-pulse"></div>Private Notes</h3>
          <button onClick={() => setIsAddingNote(!isAddingNote)} className="text-xs font-black text-brand-400 hover:text-brand-300 uppercase tracking-widest flex items-center gap-2 transition-colors font-bold">{isAddingNote ? 'Discard' : <><Plus size={16} /> Add Note</>}</button>
        </div>
        <AnimatePresence>{isAddingNote && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="mb-8 overflow-hidden"><textarea className="w-full bg-slate-800/50 border-2 border-slate-700 rounded-3xl p-6 text-sm font-medium focus:border-brand-500 outline-none min-h-[120px] transition-all" placeholder="Capture your thoughts..." autoFocus value={newNoteContent} onChange={e => setNewNoteContent(e.target.value)} />
              <div className="flex justify-end mt-4"><button onClick={handleSaveNote} disabled={!newNoteContent.trim()} className="bg-brand-500 text-white px-8 py-3 rounded-2xl text-xs font-black uppercase tracking-widest disabled:opacity-30 hover:bg-brand-400 transition-all active:scale-95 shadow-xl shadow-brand-500/20 flex items-center gap-3 font-bold"><Send size={14} /> Commit Note</button></div>
            </motion.div>
        )}</AnimatePresence>
        {note ? (
          <motion.div layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-slate-800/50 border border-slate-700 rounded-3xl p-8 relative group hover:border-slate-600 transition-all"><p className="text-slate-300 leading-relaxed font-medium whitespace-pre-wrap mb-8 text-lg italic font-medium">"{note.content}"</p>
            <div className="flex items-center justify-between opacity-60"><div className="flex items-center gap-3"><div className="w-8 h-8 rounded-full bg-brand-500/20 flex items-center justify-center text-[10px] font-black text-brand-400 uppercase tracking-widest border border-brand-500/20">{note.author.charAt(0)}</div><span className="text-[10px] font-black uppercase tracking-widest tracking-tighter">{note.author} • Lead Strategist</span></div><Pin size={16} className="text-brand-400" fill="currentColor" /></div>
          </motion.div>
        ) : (
...
const FilterButton = ({ label, active, onClick, count }: { label: string; active: boolean; onClick: () => void; count: number }) => (
  ...
  >
    {label}
    <span className={`text-[9px] font-black px-1.5 py-0.5 rounded ${active ? 'bg-brand-100 text-brand-600' : 'bg-slate-100 text-slate-400'}`}>
      {count}
    </span>
  </button>
);
...
const StatCard = ({ label, value, icon, color, isEditing, onEdit, onSave, onCancel, inputValue, onInputChange }: any) => (
  <motion.div layout whileHover={{ y: -4 }} className={`bg-slate-50/50 rounded-[32px] p-6 border border-transparent transition-all ${!isEditing && onEdit ? 'hover:bg-white hover:border-brand-100 hover:shadow-2xl hover:shadow-slate-200/50 cursor-pointer' : ''}`} onClick={!isEditing && onEdit ? onEdit : undefined}>
    ...
    <AnimatePresence mode="wait">{isEditing ? (
        <motion.div key="edit" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="flex flex-col gap-3" onClick={e => e.stopPropagation()}>
          <input type="number" autoFocus className="w-full text-xl font-black border-b-2 border-brand-500 focus:outline-none bg-transparent" value={inputValue} onChange={e => onInputChange(Number(e.target.value))} />
          <div className="flex gap-2">
            <button onClick={onSave} className="p-1.5 bg-brand-600 text-white rounded-lg active:scale-90 transition-all"><Check size={14} /></button>
