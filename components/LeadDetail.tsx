    ...
    <div className="h-full overflow-y-auto bg-white p-10 custom-scrollbar">
      <motion.div layout className="flex items-start justify-between mb-12">
            ...
            {isEditingProfile ? (
              ...
              <motion.div key="view" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} className="pt-2">
                ...
                <div className="flex flex-wrap gap-6 text-sm">
                  <div 
                    onClick={onDial}
                    className="flex items-center gap-3 px-4 py-2 bg-slate-50 rounded-2xl text-slate-600 hover:bg-indigo-50 hover:text-indigo-600 transition-all cursor-pointer group border border-transparent hover:border-indigo-100"
                  >
                  ...
                  <div 
                    onClick={onMessage}
                    className="flex items-center gap-3 px-4 py-2 bg-slate-50 rounded-2xl text-slate-600 hover:bg-fuchsia-50 hover:text-fuchsia-600 transition-all cursor-pointer group border border-transparent hover:border-fuchsia-100"
                  >
        ...
        <div className="flex items-center gap-3">
           <button onClick={() => setShowOptions(!showOptions)} className="p-3 text-slate-400 hover:bg-slate-50 rounded-2xl transition-all"><MoreHorizontal size={24} /></button>
           <AnimatePresence>{showOptions && (
             <motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }} className="absolute right-10 mt-12 w-56 bg-white rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.1)] border border-slate-50 z-50 py-3 overflow-hidden">
               <button onClick={handleStartEdit} className="w-full text-left px-6 py-3 text-sm font-bold text-slate-700 hover:bg-slate-50 flex items-center gap-3 transition-colors"><Edit2 size={18} className="text-slate-400" /> Edit Profile</button>
               <button onClick={() => {onDelete?.(lead.id); setShowOptions(false)}} className="w-full text-left px-6 py-3 text-sm font-bold text-rose-600 hover:bg-rose-50 flex items-center gap-3 transition-colors mt-1 border-t border-slate-50 pt-4"><Trash2 size={18} /> Delete Record</button>
             </motion.div>
           )}</AnimatePresence>
        </div>
      </motion.div>
      ...
      <div className="mb-12">
        <div className="flex items-center justify-between mb-8"><h3 className="text-lg font-black text-slate-900 uppercase tracking-widest flex items-center gap-3"><div className="w-2 h-2 bg-indigo-500 rounded-full"></div>Activity Feed</h3>
          <div className="flex gap-3">
            <button onClick={() => handleManualLog('call')} className="text-[10px] font-black uppercase tracking-widest bg-slate-900 text-white px-4 py-2 rounded-xl hover:bg-slate-800 transition-all active:scale-95 shadow-lg shadow-slate-900/10 flex items-center gap-2"><Phone size={12} /> Log Call</button>
            <button onClick={() => handleManualLog('email')} className="text-[10px] font-black uppercase tracking-widest bg-white border-2 border-slate-100 text-slate-900 px-4 py-2 rounded-xl hover:bg-slate-50 transition-all active:scale-95 flex items-center gap-2"><MessageSquare size={12} /> Log SMS</button>
          </div>
        </div>
        {(activities || []).length > 0 ? (
          <div className="relative pl-4 space-y-8 before:absolute before:left-4 before:top-4 before:bottom-4 before:w-px before:bg-slate-100">
            {(activities || []).map((activity, idx) => (
              <motion.div key={activity.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.1 }} className="relative pl-10">
                <div className={`absolute left-0 top-0 w-8 h-8 rounded-2xl flex items-center justify-center shadow-lg border-2 border-white translate-x-[-16px] z-10 ${activity.type === 'call' ? 'bg-indigo-600 text-white' : 'bg-fuchsia-600 text-white'}`}>{activity.type === 'call' ? <Phone size={14} /> : <MessageSquare size={14} />}</div>
      ...
      <motion.div layout className="bg-slate-900 rounded-[40px] p-10 text-white shadow-2xl shadow-indigo-900/20">
        <div className="flex items-center justify-between mb-8"><h3 className="text-lg font-black uppercase tracking-widest flex items-center gap-3"><div className="w-2 h-2 bg-indigo-400 rounded-full animate-pulse"></div>Private Notes</h3>
          <button onClick={() => setIsAddingNote(!isAddingNote)} className="text-xs font-black text-indigo-400 hover:text-indigo-300 uppercase tracking-widest flex items-center gap-2 transition-colors">{isAddingNote ? 'Discard' : <><Plus size={16} /> Add Note</>}</button>
        </div>
        <AnimatePresence>{isAddingNote && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="mb-8 overflow-hidden"><textarea className="w-full bg-slate-800/50 border-2 border-slate-700 rounded-3xl p-6 text-sm font-medium focus:border-indigo-500 outline-none min-h-[120px] transition-all" placeholder="Capture your thoughts..." autoFocus value={newNoteContent} onChange={e => setNewNoteContent(e.target.value)} />
              <div className="flex justify-end mt-4"><button onClick={handleSaveNote} disabled={!newNoteContent.trim()} className="bg-indigo-500 text-white px-8 py-3 rounded-2xl text-xs font-black uppercase tracking-widest disabled:opacity-30 hover:bg-indigo-400 transition-all active:scale-95 shadow-xl shadow-indigo-500/20 flex items-center gap-3"><Send size={14} /> Commit Note</button></div>
            </motion.div>
        )}</AnimatePresence>
        {note ? (
          <motion.div layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-slate-800/50 border border-slate-700 rounded-3xl p-8 relative group hover:border-slate-600 transition-all"><p className="text-slate-300 leading-relaxed font-medium whitespace-pre-wrap mb-8 text-lg italic">"{note.content}"</p>
          ...
          <div className="text-center py-12 border-2 border-dashed border-slate-800 rounded-3xl text-slate-600"><p className="text-xs font-black uppercase tracking-widest">Workspace is clear</p></div>
        )}
      </motion.div>
    </div>
  );
